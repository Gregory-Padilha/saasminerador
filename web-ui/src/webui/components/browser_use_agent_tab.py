...
def _report_activity_to_saas(data: dict):
    try:
        import urllib.request
        import json
        activity_url = os.environ.get("OFFER_MINER_ACTIVITY_URL", "http://127.0.0.1:3000/api/agent/activity")
        ...
        logger.debug(f"SaaS activity report skipped: {e}")
...
async def _handle_new_step(
    ...
    # Report live step to SaaS
    try:
        next_goal_text = ""
        if hasattr(output, "current_state") and output.current_state and hasattr(output.current_state, "next_goal"):
            next_goal_text = str(output.current_state.next_goal)
        elif hasattr(output, "model_output") and output.model_output and hasattr(output.model_output, "current_state"):
            next_goal_text = str(output.model_output.current_state.next_goal)
        _report_activity_to_saas({
            "status": "RUNNING",
            "current_step": step_num + 1,
            "next_goal": next_goal_text or f"Executando passo {step_num + 1} no navegador...",
        })
    except Exception as e:
        logger.debug(f"Error reporting step activity: {e}")
...
def _handle_done(webui_manager: WebuiManager, history: AgentHistoryList):
    ...
    # Notify SaaS of completion
    _report_activity_to_saas({
        "status": "COMPLETED",
        "next_goal": "Mineração concluída pelo navegador autônomo!",
    })
    ...
    if final_result:
        ...
        try:
            import urllib.request
            stage_endpoint = os.environ.get("OFFER_MINER_STAGE_URL", "http://127.0.0.1:3000/api/agent/stage")
            req = urllib.request.Request(
                stage_endpoint,
                data=json.dumps({"rawText": str(final_result)}).encode("utf-8"),
                headers={"Content-Type": "application/json"},
                method="POST",
            )
            with urllib.request.urlopen(req, timeout=4) as resp:
                stage_res = json.loads(resp.read().decode("utf-8"))
                if stage_res.get("success") and stage_res.get("count", 0) > 0:
                    final_summary += f"\n✨ **Offer Miner:** {stage_res.get('count')} oferta(s) minerada(s) enviada(s) para conferência e aprovação no SaaS!\n"
        except Exception as e:
            logger.debug(f"Auto-stage on task completion skipped or no offers: {e}")
...
async def run_agent_task(
    ...
    _report_activity_to_saas({
        ...
        "max_steps": 100,
        "resetStartTime": True,
        "next_goal": "Iniciando varredura no navegador e conectando à página alvo...",
    })
