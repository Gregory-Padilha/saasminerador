...
export async function GET(req: NextRequest) {
  ...
  if (searchParams.has('category')) rawArgs.category = searchParams.get('category');
  if (searchParams.has('offerId')) rawArgs.offerId = searchParams.get('offerId');
  if (searchParams.has('limit')) rawArgs.limit = Number(searchParams.get('limit'));

  try {
    const data = await executeAiTool('search_insights', rawArgs);
